...
def guide_callback(bot, update):
    ...
    if callback.qact == 'keyboard' and callback.qdata == 'close':
        ...
        if callback.qdata == '1':
            # check bot is admin
            if is_admin(bot, update, (query.message.chat.id, bot.id)) == False:
                query.answer(_('請給予 bot 管理員權限（delete 以及 ban）'),
                             show_alert=True)
                return
            ...
            text = f'<code>{escape(query.message.chat.title)}</code>\n' + \
                ...
                _('❌ - 關閉訂閱')
            keyboard = generate.inline_groupconfig(
                bot, update, 0, inputqtype='guide')
            query.edit_message_text(
                text, reply_markup=keyboard, parse_mode='html')
        elif callback.qdata == '3':
            text = f'<code>{escape(query.message.chat.title)}</code>\n' + \
                _('⚙️ 附加功能設定\n') + \
                _('✅ - 開啟訂閱\n') + \
                _('❌ - 關閉訂閱')
            keyboard = generate.inline_groupconfig(
                bot, update, 1, inputqtype='guide')
            query.edit_message_text(
                text=text, reply_markup=keyboard, parse_mode='html')
